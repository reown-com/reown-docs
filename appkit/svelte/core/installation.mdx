...
<Note>
  We recommend using [SvelteKit v5](https://kit.svelte.dev/) to get started with Reown AppKit for Svelte.
</Note>
...
The recommended way to trigger the modal in Svelte is to use the `<appkit-button />` web component. After setting up Reown in your application, you can simply use the button component anywhere in your Svelte templates:
...
The `<appkit-button />` is a web component that's automatically registered when Reown is initialized. No additional imports are required.

### Alternative approaches

You can also trigger the modal programmatically using the Reown AppKit instance:
...
Or use other Reown web components:
