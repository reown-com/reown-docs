...
Reown provides a comprehensive set of Vue composables that work together to provide a complete wallet connection and blockchain interaction experience. These composables can be categorized into several functional groups:
...
The diagram below illustrates how these composables relate to each other and to the core Reown functionality:
...
- `stateData.initialized`: Boolean that indicates if Reown has been initialized. This sets to true when all controllers, adapters and internal state is ready
- `stateData.loading`: Boolean that indicates if Reown is loading
