...
Reown provides a simple solution for integrating with "Sign In With Ethereum" (SIWE), a form of authentication that enables users to control their digital identity with their Ethereum account. SIWE is a standard also known as [EIP-4361](https://docs.login.xyz/general-information/siwe-overview/eip-4361).
...
## Configure your Reown Client

To integrate SIWE with Reown, you need to configure your Reown client using `Modal.Model.AuthPayloadParams`, which are required to create a SIWE message for the user to sign:
...
If the wallet connecting to your dapp does not support One-Click Auth, the SDK will fallback to the `wc_sessionPropose` method and create a session with the wallet. Reown will then inform the user that they need to sign a message to prove address ownership. Reown will send a SIWE request to the wallet, and once the wallet responds with a signed message, use `onSIWEAuthenticationResponse` callback to check the result:
...
The wallet will also send responses using app links. Your app needs to pass these responses to the Reown client so it can process them.
