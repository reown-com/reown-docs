...
description: A guide to help you migrate your existing Reown React Native integration to the new multichain version.
---

This guide will walk you through the necessary steps to update your existing Reown React Native integration to the latest multichain version. The new version introduces a more flexible and powerful architecture, centered around a core Reown library and distinct chain-specific adapters.
...
If you were previously using older, specific Reown packages like `@web3modal/*` or potentially differently named predecessors to `@reown/appkit-*-react-native`, you should remove them. The primary packages to focus on now are `@reown/appkit-react-native` and the new chain adapters.

### b. Install Core Package & Polyfills/Helpers
Install the main Reown library along with essential peer dependencies for React Native:
...
## 3. Reown Initialization (`createAppKit`)

The way you initialize Reown has changed significantly to support the new adapter-based architecture.
...
-   The `networks` array in `createAppKit` takes `AppKitNetwork` objects, which define all supported networks for Reown's UI and context. `viem/chains` objects can be directly used for EVM networks.
...
All Reown hooks are now imported directly from `@reown/appkit-react-native`.
...
-   **`useAppKitEventSubscription()` (New Hook)**:
    -   Import from `@reown/appkit-react-native`.
    -   Allows subscription to various Reown lifecycle events.
-   **Deprecated Hooks:**
