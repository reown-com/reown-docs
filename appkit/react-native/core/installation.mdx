...
Reown for React Native enables seamless integration with multiple blockchain ecosystems, including **EVM chains (like Ethereum, Polygon, etc.), Solana, Bitcoin, and TON**. It provides a unified API to manage wallet connections, interact with different chains, and build rich multi-chain applications.

At the core of Reown is a centralized `AppKit` instance that handles connection logic, manages various chain adapters, and provides data to your application via a React Context.
...
Get started quickly with our dedicated CLI that sets up a minimal Reown AppKit configuration for you.
...
### Add Reown to your existing project

Installation is a two-step process:

1. Install the core Reown library.
  ...
  <Tab title="EVM">
    ...
    <Tabs>
      ...
      <Tab title="Wagmi">
        ...
        <p>First, install the Reown AppKit Wagmi adapter and its peer dependencies:</p>
        ```bash
        ...
        **Yarn users:**
        ...
        <p>For a practical implementation example, you can refer to the <a href="https://github.com/reown-com/appkit-react-native/blob/develop/apps/native/App.tsx" target="_blank\">Reown + Wagmi example on GitHub</a>.</p>
      </Tab>
    </Tabs>
...
Before setting up Reown, you need to wrap your app with `SafeAreaProvider` from `react-native-safe-area-context`. This is required for proper modal rendering and safe area handling.
...
<Note>
  **Default Features:** Reown comes with email and social login, swaps, and onramp features enabled
...
### 5. Using Reown in Components

You can now access Reown functionalities (like connecting, getting account state, etc.) using hooks provided by the library.
...
Reown includes pre-built components to speed up your development:
...
All 600+ wallets in the Reown ecosystem work via WalletConnect protocol regardless of this configuration. You only need to add the wallets your users most commonly have installed.

</Info>

To enable Reown to detect wallets installed on the device, you can make specific changes to the native code of your project.
...
To enable Reown to detect wallets installed on the device in your Expo project for iOS, follow these steps:
...
<Tab title="Android">
To enable Reown to detect wallets installed on the device in your Expo project for Android, follow these steps:
...
<Info>
  **Optional feature** - These connectors are only needed if you want to support specific wallets
  that use custom connection protocols. All 600+ wallets in the Reown ecosystem work via
  WalletConnect protocol regardless of this configuration.
</Info>

Reown supports various custom connectors for specific wallets that use their own connection protocols. These connectors allow you to integrate popular wallets like Coinbase and Phantom directly into your Reown AppKit configuration.

### Coinbase

Coinbase Wallet is a popular wallet that uses a custom connection protocol. To enable it in your Reown AppKit configuration, you'll need to install the Coinbase SDK and add a specific connector.
...
Phantom and Solflare are popular wallets on the Solana blockchain. Because they use a custom connection protocol, it requires a specific connector to be added to your Reown AppKit configuration.
...
<Tab title="Wagmi">
  <Card
    title="Reown with Wagmi example"
    icon="github"
...
<Tab title="Ethers">
  <Card
    title="Reown with Ethers example"
    icon="github"
...
<Tab title="Ethers v5">
  <Card
    title="Reown with Ethers v5 example"
    icon="github"
