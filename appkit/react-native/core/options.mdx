...
Reown for React Native is configured by calling the `createAppKit` function and passing a configuration object to it. This function initializes and returns the Reown AppKit instance that you will use throughout your application, typically by providing it via the `AppKitProvider`.
...
<Accordion title="Pre-exported by Reown">
For convenience, Reown exports configurations for Solana and Bitcoin networks.
...
*   **Type:** `Array<Adapter>`
*   **Required:** Yes
*   **Description:** An array of initialized chain adapter instances. Each adapter is responsible for handling a specific blockchain namespace (e.g., 'eip155' for EVM chains, 'solana' for Solana). The SDK will map the networks provided in the `networks` option to the appropriate adapter based on the namespace derived from the network's `id` and the namespaces supported by each adapter.
...
*   **Type:** `Storage`
*   **Required:** Yes
*   **Description:** A storage implementation that handles data persistence. Reown provides a flexible storage interface that works with any storage solution you prefer.
...
*   **Required:** No
*   **Description:** Additional wallet connectors that extend Reown's connection capabilities beyond the standard WalletConnect protocol. This is useful for wallets that require custom connection methods, like Phantom Wallet and Coinbase.
...
*   **Description:** The network that Reown should attempt to connect to by default, or highlight initially if no prior session exists. This object must be one of the networks defined in your `networks` array.
...
*   **Description:** Sets the initial theme for the Reown modal. Defaults to the system theme or 'light' if system theme is unavailable.
...
*   **Description:** Allows you to customize the accent color of the Reown modal. Refer to the Theming documentation for details.
...
*   **Description:** An object to configure specific tokens for which Reown should display balances. The keys are CAIP-2 network IDs (e.g., 'eip155:1' for Ethereum Mainnet for a specific Solana network instance if required by ID structure). The value for each network ID is an object containing the token's contract `address` on that network.
...
*   **Description:** Allows you to provide your own clipboard implementation. **If this client is not provided, all UI elements within Reown that offer a "copy to clipboard" functionality (e.g., for addresses, QR code URIs) will be hidden.** To enable copy features, you must supply this client.
...
### `features`
*   **Type:** `Object` (Optional)
*   **Description:** An object to toggle additional Reown features on or off.
