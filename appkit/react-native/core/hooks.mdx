...
- `initialized`: Boolean that indicates if the AppKit has been initialized. This sets to true when all controllers, adapters and internal state is ready
- `loading`: Boolean that indicates if the AppKit is loading
...
function AppStatus() {
  ...
  if (!initialized) {
    return <Text>Initializing AppKit...</Text>;
  }
  ...
  return (
    <View>
      <Text>Modal is {open ? 'open' : 'closed'}</Text>
      <Text>Selected Network: {selectedNetworkId}</Text>
      <Text>Active Chain: {activeChain}</Text>
    </View>
  );
}
```

The modal state consists of five reactive values:

| State              | Description                                                                 | Type      |
|--------------------|-----------------------------------------------------------------------------|-----------|
| `initialized`      | Indicates if the AppKit has been initialized. This sets to true when all controllers, adapters and internal state is ready. | `boolean` |
| `loading`          | Indicates if the AppKit is loading.                                        | `boolean` |
| `open`             | Open state will be true when the modal is open and false when closed.       | `boolean` |
| `selectedNetworkId`| The current chain id selected by the user in CAIP-2 format.               | `CaipNetworkId` |
| `activeChain`      | The active chain namespace (e.g., 'eip155', 'solana', 'bip122').          | `ChainNamespace` |
