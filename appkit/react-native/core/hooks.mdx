...
Reown for React Native provides a set of hooks to interact with the wallet connection lifecycle, access connection state, and control the modal.
...
You can switch networks by passing either an `AppKitNetwork` object or a CAIP network ID string:

```typescript
const { switchNetwork } = useAppKit();

// Using an AppKitNetwork object
switchNetwork(polygonNetwork);

// Using a CAIP network ID string
await switchNetwork('eip155:1'); // Ethereum Mainnet
await switchNetwork('eip155:137'); // Polygon
```
...
export default function MyComponent() {
  const {
    allAccounts, // Account[], all connected accounts across all connections
  ...
  if (isConnected) {
    return (
      <View>
        <Text>Connected to chain: {chainId}</Text>
...
The provider access hook that provides access to the underlying chain-specific provider for the currently active connection. Use this hook when you need to perform direct interactions with the blockchain that go beyond Reown AppKit's standard actions.
...
- Performing advanced blockchain operations not covered by Reown AppKit's standard actions
...
export default function ThemeControl() {
  const {
    themeMode, // 'dark' or 'light', the current theme mode
    themeVariables, // Current theme variables (e.g., { accent: '#00BB7F' })
  ...
  return (
    <View>
      ...
      <Button
        ...
        onPress={() => setThemeVariables({ accent: '#FF5733' })}
      />
      <Button
        title="Reset to System Theme"
        onPress={() => setThemeMode(undefined)}
      />
    </View>
  );
}
...
The logging hook that provides access to Reown AppKit's internal logging system. Use this hook when you need to debug your application, monitor the SDK's behavior, or export logs for troubleshooting purposes.
...
### Use Cases
- Debugging the SDK's behavior and troubleshooting issues
...
<Note>
This hook requires the `debug` option to be set to `true` in your Reown AppKit configuration to capture logs. See the [Options](/appkit/react-native/core/options#debug) documentation for more details.
</Note>
