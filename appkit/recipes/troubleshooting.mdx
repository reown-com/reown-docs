...
## SvelteKit v4 Compatibility with AppKit Pay

⚠️ **Note for SvelteKit v4 Users**

AppKit Pay (`@reown/appkit-pay`) uses `@solana/web3.js`, which relies on Node.js APIs. SvelteKit v4 doesn't polyfill Node modules by default, so you may see build errors.

If you're using SvelteKit v4 and encounter this:

- **Consider upgrading to SvelteKit v5** (recommended), or
- **Add the required Node.js polyfills manually** using `vite-plugin-node-polyfills`

### Recommended Approach

AppKit Pay is designed for use with environments like SvelteKit v5 or other setups that handle Node polyfills. For the best experience and long-term compatibility, we recommend upgrading to SvelteKit v5 when possible.

### Alternative Approach (SvelteKit v4)

If you need to stay on SvelteKit v4, you can add Node.js polyfills:
