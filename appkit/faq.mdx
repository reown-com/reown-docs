...
<AccordionGroup>
  ...
  <Accordion title="Does createAppKit need to be called before rendering AppKit components in React Native?">
    Yes, in React Native applications, `createAppKit` must be called before rendering the `<AppKit />` component or any other AppKit UI components (like `<AppKitButton />`). 
    
    **Correct approach:**
    ```tsx
    // Call createAppKit at module level, outside components
    createAppKit({
      projectId,
      wagmiConfig, // or your ethers config
      // ... other options
    });
      ...
      return (
        <WagmiProvider config={wagmiConfig}>
          <QueryClientProvider client={queryClient}>
            <AppKit /> {/* This will work correctly */}
          </QueryClientProvider>
        </WagmiProvider>
      );
    }
    ...
    **Incorrect approach:**
    ```tsx
    export default function App() {
      // Don't call createAppKit inside components
      createAppKit({ /* config */ });
      
      return <AppKit />; // This may cause issues
    }
    ```
  </Accordion>
</AccordionGroup>
