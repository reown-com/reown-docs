...
<Accordion title="How do I remove the 'UX by Reown' branding?">
  Currently, only enterprise clients can hide "UX by Reown" on the AppKit modal by adjusting this
  option on our Dashboard. If you are an enterprise client and would like to hide this branding,
  ...
  <Accordion title="Is there a limit on self-custodial wallet connections?">
    There is no fixed number for what counts as "excessive" self-custodial connections. Our Terms of Service reserve the right to limit usage that is clearly abusive or disruptive, such as traffic spikes or automated spamming, and this is assessed case by case. See our [Community Licence](ttps://github.com/reown-com/appkit-react-native?tab=License-1-ov-file#readme) for clearest thresholds before paid plans are needed:
...
<Accordion title="Can I reinitialize AppKit with different network configurations?">
  Currently, `createAppKit` can only be called once during the application's lifecycle. It cannot be
  lazily initialized and then torn down for re-initialization. This means you must pass in all the
  networks you plan to support during the initial setup.
</Accordion>
  ...
  <Accordion title="How do I delete a project?">
    To delete a project from your [Reown Dashboard](https://dashboard.reown.com/):

    1. Open the **Team** dropdown at the top of the Dashboard and select the team that contains the project you want to delete.

    <Frame>
      <img src="/images/faq/team-dropdown.png" alt="Team dropdown in the Reown Dashboard" />
    </Frame>

    2. Click on the **three dots** (`...`) menu at the end of the project you want to delete.

    <Frame>
      <img src="/images/faq/delete-project.png" alt="Project options menu showing Rename and Delete actions" />
    </Frame>

    3. Click **Delete** to remove the project.

    4. Type the name of the project to confirm and click **Delete**.

    <Frame>
      <img src="/images/faq/confirm-delete-project.png" alt="Confirmation dialog to delete a project" />
    </Frame>

    <Warning>
      Deleting a project is permanent and cannot be undone. Any integrations using this project's `projectId` will stop working immediately.
    </Warning>

  </Accordion>

  <Accordion title="How do I delete a team?">
    To delete a team from your [Reown Dashboard](https://dashboard.reown.com/):

    1. Open the **Team** dropdown at the top of the Dashboard and select the team you want to delete.
    ...
    3. Click the **Delete** button at the bottom of the settings page.

    <Frame>
      <img src="/images/faq/delete-team.png" alt="Delete button on the team settings page" />
    </Frame>

    4. Type the name of the team to confirm and click **Delete**.

    <Frame>
      <img src="/images/faq/confirm-delete-team.png" alt="Confirmation dialog to delete a team" />
    </Frame>

    <Warning>
      Deleting a team is permanent and cannot be undone. All projects within the team will also be deleted, and any integrations using their `projectId` values will stop working immediately.
    </Warning>

  </Accordion>

  <Accordion title="How do I update AppKit packages to the latest version?">
    Keep every `@reown/*` package on the exact same version. AppKit publishes its packages in coordinated releases, so mixing versions can cause subtle peer-dep and type mismatches.
    ...
    All release information is available in the [AppKit GitHub Releases](https://github.com/reown-com/appkit/releases).
  ...
  <Accordion title="How does Verify API know if a website is a scam or suspicious?">
    [Verify API](https://docs.walletconnect.network/wallet-sdk/web/verify) checks the origin that is
    requesting a connection or a signature against the domains registered to that `projectId`, and it also
    checks that domain's reputation. Wallets that implement Verify API use the result to warn users
    before they approve a request from an unverified or malicious site.

    The reputation data comes from three security providers whose threat intelligence is combined to
    classify a domain as a scam or suspicious:

    - [Hexagate](https://hexagate.com)
    - [ChainPatrol](https://chainpatrol.io)
    - [Hypernative](https://www.hypernative.io)

    As a Web3 app, the best way to avoid being flagged is to keep your allowed domains up to date in the
    [Reown Dashboard](https://dashboard.reown.com) so your origin always resolves as verified.
...
<AccordionGroup>
  <Accordion title="How do I migrate from WalletConnect Modal to Reown AppKit?">
    Follow our dedicated guide to upgrade from WalletConnect Modal to Reown AppKit:

    - [Migrate from WalletConnect Modal to Reown AppKit](/appkit/upgrade/wcm)
  </Accordion>

  <Accordion title="How do I migrate from Web3Modal to Reown AppKit?">
    Pick the guide that matches your platform:

    **Web**

    - [Reown AppKit (Wagmi)](/appkit/upgrade/to-reown-appkit-web)
    - [Reown AppKit (Ethers)](/appkit/upgrade/to-reown-appkit-ethers-web)
    - [Reown AppKit (Ethers v5)](/appkit/upgrade/to-reown-appkit-ethers5-web)
    - [Reown AppKit (Solana)](/appkit/upgrade/to-reown-appkit-solana-web)

    **Other platforms**

    - [React Native](/appkit/upgrade/from-web3modal-react-native)
    - [Flutter](/appkit/upgrade/from-web3modal-flutter)
    - [Android](/appkit/upgrade/from-web3modal-android)
    - [iOS](/appkit/upgrade/from-web3modal-ios)
    - [Unity](/appkit/upgrade/from-web3modal-unity)
  </Accordion>

  <Accordion title="How do I migrate from other libraries to Reown AppKit?">
    Follow the guide for the library you're migrating from:

    - [Migrating from Privy to AppKit](/appkit/migration/from-privy-next)
    - [Migrating from RainbowKit to AppKit](/appkit/migration/from-rainbowkit-next)
    - [Migrating from ConnectKit to AppKit](/appkit/migration/from-connectkit-next)
    - [Migrating from Solana Anza Adapter to AppKit](/appkit/migration/from-anza-adapter-react)
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="How do I get technical support for AppKit?">
    Go to the [Reown Dashboard](https://dashboard.reown.com) and contact us through the Intercom bubble.

    Anyone can [report an issue](/advanced/report-an-issue), and customers on any paid plan can contact our support engineers.
