...
<Accordion title="How do I remove the 'UX by Reown' branding?">
  ...
  <Accordion title="Is there a limit on self-custodial wallet connections?">
    ...
    **Using your own RPC provider?** The RPC threshold will likely not apply to you. RPC usage may still occur from time to time to fetch users balances. Likely the only limit that could apply is the general fair use clause in our ToS. If your usage could reasonably be described as a spike attack or sustained automated abuse, that's when it becomes a concern. Normal app traffic is not what this clause is designed for.
  </Accordion>
</AccordionGroup>
