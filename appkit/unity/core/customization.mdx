...
The SDK uses Unity's [UI Toolkit](https://docs.unity3d.com/Manual/UIElements.html) system.
By following this guide,you can customize Reown to match your game's style, even if you have minimal or no prior
...
Reown relies heavily on [Unity Style Sheet (USS) variables](https://docs.unity3d.com/Manual/UIE-USS-variables.html)
for easy customization of various UI aspects, such as colors, spacing, border radius, and more.

### Preparation

The first step in theming Reown is to create a custom panel settings asset, followed by a custom theme file.
Then, link them to Reown.
...
4. In the same Inspector window, set `Reference DPI` to 160. This is the recommended DPI for Reown, but feel free
...
Everything is set up, now you can start customizing Reown to match your game's style.
...
Reown follows the MVP (Model-View-Presenter) architecture, allowing you to customize the UI and behavior by
...
All core views in Reown accept optional path to the UXML layout file in the constructor.
