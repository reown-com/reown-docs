...
The Reown uses Unity's [UI Toolkit](https://docs.unity3d.com/Manual/UIElements.html) system.
By following this guide,you can customize the Reown to match your game's style, even if you have minimal or no prior
...
The Reown relies heavily on [Unity Style Sheet (USS) variables](https://docs.unity3d.com/Manual/UIE-USS-variables.html)
for easy customization of various UI aspects, such as colors, spacing, border radius, and more.

### Preparation

The first step in theming the Reown is to create a custom panel settings asset, followed by a custom theme file.
Then, link them to the Reown.
...
4. In the same Inspector window, set `Reference DPI` to 160. This is the recommended DPI for the Reown, but feel free
...
Everything is set up, now you can start customizing the Reown to match your game's style.
...
You can find the list of all available variables in the [Reown Base Theme](https://github.com/reown-com/reown-dotnet/blob/main/src/Reown.Reown.Unity/UI%20Toolkit/AppKit%20Base%20Theme.tss)
file.
...
The Reown follows the MVP (Model-View-Presenter) architecture, allowing you to customize the UI and behavior by
...
All screens in the Reown modal (e.g. Account, Wallet Search, QR Code, etc.) are called `modal view`. The class that
...
All core views in the Reown accept optional path to the UXML layout file in the constructor.
