...
To enable Reown to detect wallets installed on mobile devices, you need to make specific changes to the native side of the project.
...
To fix this, the SDK creates a container around the canvas node and appends the modal UI to it.
...
To ensure reliability, particularly on iOS, it is recommended to register a custom URL scheme and provide it to Reown.
