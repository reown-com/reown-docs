...
**Deposit with Exchange** gives your users a seamless way to fund their connected wallets directly from centralized exchange accounts like **Coinbase** and **Binance** (with more coming soon).
...
4. Completes the deposit/transfer flow via the CEX's authorization.
...
If a token or network is supported by the exchange, it can be enabled. EVM networks are especially quick to set up and can be activated on demand.
...
🔜 Coming soon: Robinhood, Kraken, Bybit, Bitget, OKX, plus native gas & stablecoins.

**Platforms & SDKs**
- React, Next.js, Vue, JavaScript SDK
- Mobile web & native apps (via deep link + QR)

## Use cases

### Dapps (DeFi, Staking, Restaking)

- Enable users to fund → stake in one smooth flow (experimental).
