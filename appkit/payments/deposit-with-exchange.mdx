...
<Warning>
  Coinbase has recently updated their compliance requirements. AppKit Deposit with Exchange remains compliant and will be re-enabled once partners provide their own CDP API keys. We are currently implementing support for partners to connect their own Coinbase Developer Platform (CDP) API keys.
</Warning>
