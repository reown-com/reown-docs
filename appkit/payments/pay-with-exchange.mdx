...
**AppKit Pay with Exchange** unlocks a powerful new flow: users can pay in crypto directly from their **Centralized Exchange (CEX) accounts** like Binance, **with no new wallets, no app switching, and no lost conversions.** This flow is bundled together with the **Pay with Self-Custodial Wallets** flow.

<Warning>
    Currently, Binance is the only exchange compatible with the Pay with Exchange flow, but we will be adding more centralized exchanges in future.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Which exchanges are supported?">
    Currently, Binance is the only exchange compatible with the Pay with Exchange flow, but we will be adding more centralized exchanges in future. Contact us [via our form here](https://share.hsforms.com/1_oWa8QkwRXi6oR0nZ_SIxQnxw6s) to inquire.
  </Accordion>
