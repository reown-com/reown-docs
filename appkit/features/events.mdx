...
`useAppKitEvents` (React / Next) and `useAppKitEvents` (Vue / Nuxt composable) subscribe to every event emitted by AppKit.
Use them for analytics, telemetry, custom notifications, or reacting to specific user actions.
  ...
  useEffect(() => {
    if (event.data.event === 'CONNECT_SUCCESS') {
      analytics.track('wallet_connected', {
        wallet: event.data.properties.name,
        method: event.data.properties.method
      })
    }
  }, [event])
...
export function NetworkToast() {
  ...
  useEffect(() => {
    if (event.data.event === 'SWITCH_NETWORK') {
      toast(`Switched to ${event.data.properties.network}`)
    }
  }, [event])
...
export function EventLogger() {
  ...
  useEffect(() => {
    ...
    console.log(`[AppKit] ${event.data.event}`, event.data.properties)
  }, [event])
  ...
  useEffect(() => {
    switch (event.data.event) {
      ...
      case 'CONNECT_PROXY_ERROR':
        toast.error(`Connection failed: ${event.data.properties?.message}`)
        break
      case 'USER_REJECTED':
