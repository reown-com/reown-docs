---
title: Migration from Privy to AppKit
sidebarTitle: "From Privy"
---

<Note> 
If you're migrating a React Privy dApp originally built with `create-react-app`, we recommend switching to **Vite** for better performance and developer experience. 
You can check out our [examples](https://github.com/reown-com/appkit-web-examples) for a full implementation using **Vite + AppKit**. 
</Note>
...
<CodeGroup>
```bash npm
npm install @reown/appkit @reown/appkit-adapter-wagmi && npm uninstall @privy-io/react-auth @privy-io/server-auth
```
```bash Yarn
yarn add @reown/appkit @reown/appkit-adapter-wagmi&& yarn remove @privy-io/react-auth @privy-io/server-auth
```
```bash Bun
bun add @reown/appkit @reown/appkit-adapter-wagmi && npm uninstall @privy-io/react-auth @privy-io/server-auth
```
```bash pnpm
pnpm add @reown/appkit @reown/appkit-adapter-wagmi && pnpm remove @privy-io/react-auth @privy-io/server-auth
```
</CodeGroup>
