...
sidebarTitle: Reown Solana
---

# Upgrade Guide

## Web3Modal v5 to Reown - Web | Solana
...
Next, install the Reown and Solana wallet adapter packages.
...
The `chains` property is now `networks` in Reown. You should import them from `@reown/appkit/networks` package instead of importing these networks other packages.
...
- (Reown v1) - switchNetwork(solanaDevnet) -> `solanaDevnet` is imported from `@reown/appkit/networks`.
