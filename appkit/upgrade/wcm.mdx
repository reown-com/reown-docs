---
title: WalletConnect Modal to Reown Core
---

**Reown Core is the upgraded version of WalletConnect Modal.** It provides a user-friendly experience while maintaining the familiar QR code functionality for wallet connections. **Reown Core** is the base version of Reown that includes the traditional WalletConnect Modal with QR code functionality.

<Note>This is a preview version of Reown Core.</Note>
...
<Note>
If your project has `@walletconnect/modal` in the `package.json` or your project files, you need to remove it and uninstall it. After that, you can refer to the [Universal Provider path](#universal-provider) to setup Reown Core in your project.
</Note>

 **Reown Core** is the most basic version of Reown which replaces WalletConnect Modal. Please refer to [this section](#setting-up-appkit-core-from-scratch) if you are starting from scratch.

## Installation

You first need to install the Reown package in order to get started. You can do this by running the command below.
...
Then, you can use the following code to configure Reown with `UniversalProvider`.
...
WalletConnect Modal has always been chain agnostic. Reown Core is chain agnostic as well. Hence, you can configure custom networks like Polkadot, Cosmos, etc., using Reown Core.
...
Then, you can use the following code to configure Reown with `SignClient`.
...
## Setting up Reown Core from scratch
