...
- `multiWallet`: Boolean that indicates if multi-wallet functionality is enabled (requires paid plan activation)
...
<Note>
  Multi-wallet functionality must be enabled in your AppKit configuration. This is a paid feature that requires activation in your Reown Cloud dashboard.
</Note>
...
function WalletSwitcher() {
  ...
  const { 
    ...
    onSuccess({ hasSwitchedWallet, hasDeletedWallet }) {
      if (hasDeletedWallet) {
        alert('Wallet removed from history');
      } else if (hasSwitchedWallet) {
        alert('Successfully switched wallet');
      }
    },
    onError(error) {
