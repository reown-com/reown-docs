...
To open the modal you can use our [**React components**](/appkit/react/core/components) or build your own button with Reown [**hooks**](/appkit/react/core/hooks#useAppKit).
...
After providing the project name and selecting your preferences, the CLI will install a minimal example of Reown AppKit with your preferred blockchain library. The example will be pre-configured with a `projectId` that will only work on `localhost`.
