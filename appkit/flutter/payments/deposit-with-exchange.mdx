...
After enabling it, __Deposit with Exchange__ is accessible through Account Screen by clicking on "Fund wallet" button and the modal below will be shown:
...
```
___supportedAssets:___ (optional) previous selected supported assets<br />
...
Create a `ReownAppKit` instance and initiate (as for any other AppKit usage)
...
Currently the feature works with a set of supported assets. You can use the following function to filter them out according or your needs….
...
Or you can actually pick any of the preconfigured constants by looking at their definition…
...
Whether you previously selected a supported `asset` or not, next step is to get all the supported exchanges. If `asset` is provided, the response will include only the exchanges supporting that particular asset
...
`GetExchangesParams` also accepts an `includeOnly` and exclude parameters to filter out the `getExchanges` response. Example:
...
final GetExchangesResult result = await appKit.getExchanges(params: params);
// result.exchanges will only contain Coinbase;
// result.total = 1;
```

`GetExchangesResult` contains a `List<Exchange>` exchanges and the total amount of them.
...
At this point your dApp will launch this URL in the user device’s and the Exchange App will be launched if installed, otherwise their default browser to log In into this exchange and fund.

### Status Check

Once the user completes the transaction on the Exchange side, it should return to your dApp and wait for the confirmation status.

> The recommended approach here is that, right after the user launches the payment URL your dApp starts to query the confirmation status in a loop with 5 seconds delay between each request, until either **CONFIRMED** or **FAILED** is received (or your own timeout logic is triggered)
...
- **UNKNOWN:** Transaction still not submitted
