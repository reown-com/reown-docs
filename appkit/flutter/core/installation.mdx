...
In order to support Solflare Wallet interactions a few extra steps has to be performed (only if you haven't implemented [Link Mode](./link-mode) already).
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
    ...
    <queries>
        <package android:name="com.phantom"/>
        <!-- Any other scheme previously added -->
    </queries>
...
The rest of the configuration is identiacal to Phantom's

#### Disable Solflare Wallet

Solflare Wallet is enabled by default even if, in order to function properly, a few steps has to be done as described in the previous section. However, if you don't want to include/support Solflare Wallet on your app you just need to pass Solflare Wallet id `1ca0bdd4747578705b1939af023d120677c64fe6ca76add81fda36e350605e79` to [excludedWalletIds](./options#excludedwalletids) options Array.
