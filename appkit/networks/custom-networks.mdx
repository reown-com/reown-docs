...
Proxies a request to the Canton Ledger API and returns its response verbatim. The `params` object is forwarded as-is to the underlying Ledger API endpoint.
...
<Info>The result field contains the raw Ledger API JSON response as-is.</Info>
