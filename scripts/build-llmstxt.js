 ...
 * LLMSTXT spec (https://llmstxt.org/). It generates four files:
...
function generateLLMSTXTIndex(processedFiles) {
  ...
  llmstxt += `## Specialized Documentation\n\n`
  llmstxt += `For more focused documentation, see these specialized LLMSTXT files:\n\n`
  llmstxt += `- [Complete Documentation](${SITE_URL}/llms-full.txt): Full documentation for all Reown components\n`
  llmstxt += `- [AppKit Documentation](${SITE_URL}/llms-appkit.txt): Documentation focused on AppKit\n`
  llmstxt += `- [WalletKit Documentation](${SITE_URL}/llms-walletkit.txt): Documentation focused on WalletKit\n\n`
...
async function main() {
  try {
    ...
    const appkitFiles = validFiles.filter(
      file => file.directory.startsWith('appkit') || file.path.startsWith('appkit')
    )

    const walletkitFiles = validFiles.filter(
      file => file.directory.startsWith('walletkit') || file.path.startsWith('walletkit')
    )

    console.log(
      `Found ${appkitFiles.length} AppKit files and ${walletkitFiles.length} WalletKit files.`
    )
    ...
    // Generate AppKit-specific documentation file
    const appkitLLMSTXT = generateLLMSTXT(
      appkitFiles,
      'Reown AppKit Documentation',
      'Documentation for Reown AppKit'
    )

    // Generate WalletKit-specific documentation file
    const walletkitLLMSTXT = generateLLMSTXT(
      walletkitFiles,
      'Reown WalletKit Documentation',
      'Documentation for Reown WalletKit'
    )
    ...
    fs.writeFileSync(path.join(OUTPUT_DIR, 'llms-appkit.txt'), appkitLLMSTXT)
    fs.writeFileSync(path.join(OUTPUT_DIR, 'llms-walletkit.txt'), walletkitLLMSTXT)
    ...
    console.log(`- ${path.join(OUTPUT_DIR, 'llms-appkit.txt')}`)
    console.log(`- ${path.join(OUTPUT_DIR, 'llms-walletkit.txt')}`)
  } catch (error) {
