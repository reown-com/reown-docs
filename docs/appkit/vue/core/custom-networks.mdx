...
We use Viem to provide EVM chains to users under the hood. If your chain is EVM-compatible, we recommend opening a PR to Viem to add your network to Viem’s directory. Once your chain is accepted by Viem, it will automatically be available in AppKit with no additional steps required.
