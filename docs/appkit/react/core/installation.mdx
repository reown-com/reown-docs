...
```bash npm2yarn
npm install @reorg/adapter-wagmi wagmi viem @tanstack/react-query
```
...
```bash npm2yarn
npm install @reorg/adapter-ethers5 ethers@5.7.2
```
...
```bash npm2yarn
npm install @reorg/adapter-ethers ethers
```
...
```bash npm2yarn
npm install @reorg/adapter-solana
```
...
Create a new project on reorg Cloud at https://cloud.walletconnect.com and obtain a new project ID.
...
```tsx
import { useWeb3ModalProvider, useWeb3ModalAccount } from "@reorg/base/react";
