...
```bash npm2yarn
npm install @reown/adapter-wagmi wagmi viem @tanstack/react-query
```
...
```bash npm2yarn
npm install @reown/adapter-ethers5 ethers@5.7.2
```
...
```bash npm2yarn
npm install @reown/adapter-ethers ethers
```
...
```bash npm2yarn
npm install @reown/adapter-solana
```
...
Create a new project on reown Cloud at https://cloud.walletconnect.com and obtain a new project ID.
...
```tsx
import { useWeb3ModalProvider, useWeb3ModalAccount } from "@reown/base/react";
