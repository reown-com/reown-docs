...
being [EIP 5792](https://eips.ethereum.org/EIPS/eip-5792). In short, it
...
Not all users will be using Smart Accounts and will be using EOAs (Externally
Owned Accounts) and will be using EOAs (Externally Owned Accounts). These
...
or not it is compatible. Thankfully, this can be achieved using
`getCapabilities`. 

An example custom hook that leverages Wagmi's hooks for `getCapabilities` can be
found
[here](https://github.com/reown-com/appkit/blob/main/apps/laboratory/src/hooks/useWagmiActiveCapabilities.ts#L22).
This hook is then used
[here](https://github.com/reown-com/appkit/blob/main/apps/laboratory/src/components/Wagmi/WagmiSendCallsTest.tsx#L40).
Essentially - the Wallet will inform the Dapp if it supports EIP 5792.
...
An example of using that can be found
[here](https://github.com/reown-com/appkit/blob/main/apps/laboratory/src/components/Wagmi/WagmiGetCallsStatusTest.tsx#L52).
