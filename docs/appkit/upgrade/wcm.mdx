...
```bash npm2yarn
npm install @reown/appkit@1.6.7-core-2.0
```
:::note
...
// OLD IMPLEMENTATION - Remove the Code Below
...
// NEW IMPLEMENTATION - Add the Code Below
/*highlight-add-start*/
import { UniversalProvider } from '@walletconnect/universal-provider'
...
Upgrade to AppKit Core and pass in the `UniversalProvider`. 
...
```javascript
//Old Code - Remove the Code Below
/* highlight-delete-start */
const modal = new WalletConnectModal({
  projectId: 'YOUR_PROJECT_ID',
  chains: ['eip155:1', 'solana:mainnet']
