...
Furthermore, Phantom Wallet deep/universal links mechanism supports interaction only with Solana netowkr, meaning that if you have EVM networks configured in your AppKit's instance these are not going to be available to use after connecting with Phantom.
