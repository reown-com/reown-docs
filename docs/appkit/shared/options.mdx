...
```ts
createWeb3Modal({ adapters, projectId, caipNetworks, ...options })
```
...
```ts
import { mainnet } from '@reorg/base/chains'
...
```ts
import { solana } from '@reorg/base/chains'
...
const adapter = new EVMWagmiClient({
  //...
  connectors: [
    coinbaseWallet({
      //...
