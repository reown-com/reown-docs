...
<Note>
  `@walletconnect/modal` is now deprecated. Please use `@reown/appkit` instead. For migration guidance, see the [WalletConnect Modal to AppKit Core migration guide](/appkit/upgrade/wcm).
</Note>
...
Dapps will also need to install Reown for the UI.
