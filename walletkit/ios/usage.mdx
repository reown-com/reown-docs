...
**For parallel testing scenarios** where you need isolated Core instances, you can configure a unique storage prefix through the Core configuration. The iOS SDK handles Core instance management internally through the `WalletKit.configure()` method.

<Warning>
Don't use randomly generated storage prefixes in production - this will cause the client to create new storage each time it is initialized. The client will not be able to persist/read existing data and all existing sessions will be lost after each reload.
</Warning>

<Note>
The global Core sharing behavior was introduced to prevent resource waste when multiple SDK instances are created. If you're running parallel tests and experiencing cross-test interference, consult the iOS SDK documentation for Core isolation options or contact support for platform-specific guidance.
</Note>
