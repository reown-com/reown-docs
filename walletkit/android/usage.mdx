...
**For Android SDK**, Core instance management is handled internally through the SDK's internal database system. Unlike other platforms, Android does not use storage prefixes for Core isolation.

**For parallel testing scenarios** where you need isolated Core instances on Android, consult the Android SDK documentation for platform-specific isolation methods or contact support for guidance on proper test setup.

<Note>
The global Core sharing behavior was introduced to prevent resource waste when multiple SDK instances are created. The Android SDK manages Core instances differently than other platforms due to its internal database architecture.
</Note>
