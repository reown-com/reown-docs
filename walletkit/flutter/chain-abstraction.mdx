...
If you are looking to trigger Chain Abstraction during a eth_sendTransaction Session Request you should do it inside the session request handler as explained in [Responding to Session requests](/walletkit/flutter/usage#responding-to-session-requests) section.
...
You can also use the [AppKit laboratory](https://appkit-lab.reown.com/appkit/?name=wagmi) and try sending [USDC/USDT](/walletkit/features/chain-abstraction#what-are-the-supported-tokens-and-networks%3F) with any chain abstraction-supported wallet. 
