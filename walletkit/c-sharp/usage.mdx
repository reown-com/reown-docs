...
**For parallel testing scenarios** where you need isolated Core instances, you can use a unique storage prefix:
...
var options = new CoreOptions()
{
    ProjectId = "...",
    Name = "my-app",
    CustomStoragePrefix = $"test-{DateTimeOffset.UtcNow.ToUnixTimeMilliseconds()}" // Unique prefix for each test
}
...
<Note>
The global Core sharing behavior was introduced to prevent resource waste when multiple SDK instances are created. If you're running parallel tests and experiencing cross-test interference, use the `CustomStoragePrefix` option to ensure proper test isolation.
</Note>
