---
title: "[WalletKit Web SDK] Wallet Call API"
searchTitle: "[WalletKit Web SDK] Wallet Call API"
---
...
This capability is expressed per chain and is crucial for determining how `wallet_sendCalls` with `atomicRequired: true` will be handled.
  ...
  <Accordion title="wallet_sendCalls">
    ...
    - When using Wallet Service to get calls status from the bundler (see [wallet_getCallsStatus](#wallet_getcallsstatus)), the returned batchId must be a base64 encoded JSON object with `chain_id` and `user_op_hash` fields, example:
      ```json
      {
        "chain_id": "0x01",
        "user_op_hash": "0xffff"
