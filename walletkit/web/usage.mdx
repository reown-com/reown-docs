...
```
https://{YOUR_WALLET_URL}/wc?uri={WC_URI}
```

Similarly, when clicking the Sign Message button, it will redirect to the wallet with `requestId` and `sessionTopic` in the query parameters:

```
https://{YOUR_WALLET_URL}/wc?requestId={REQUEST_ID}&sessionTopic={SESSION_TOPIC}
```
