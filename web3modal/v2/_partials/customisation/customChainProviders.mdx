Wagmi requires that one or more providers (RPCs) are configured that support all your target chains. Reown provides WalletConnect RPC via `w3mProvider` helper.
