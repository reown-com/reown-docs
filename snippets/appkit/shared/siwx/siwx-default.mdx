...
Here is a quick guide to get started with the **SIWX** feature using the `SIWXDefault` class.
...
import { SIWXDefault } from '@reown/appkit-siwx'
...
const appkit = createAppKit({
  ...
  metadata,
  // Add the following code line
  siwx: new SIWXDefault() // add this line to enable SIWX
})
```
...
## Customizing the SIWXDefault Class

The `SIWXDefault` class is divided in three main components: `SIWXMessenger`, `SIWXVerifier` and `SIWXStorage`. The `@reown/appkit-siwx` package have defined options to fulfill the parts when initializing the `DefaultSIWX` configuration and you are also able to setup your own parts as required.
