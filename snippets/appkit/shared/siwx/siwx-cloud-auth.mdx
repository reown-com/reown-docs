...
With Reown Authentication you will be able to see and control the sessions of your users using the [User Management Dashboard](#user-management).

<Warning>

For now, Reown Authentication will only work for `eip155` compatible networks.
...
<Note>
`ReownAuthentication` is the preferred class name as of AppKit v1.7.13. The previous `CloudAuthSIWX` class is still supported for backwards compatibility.
</Note>

## User Management

Easily view and manage all authenticated users through AppKit [Reown Authentication](/appkit/authentication/siwx/siwx-cloud-auth), a drop-in hosted SIWX server that provides key insights into your user base.
