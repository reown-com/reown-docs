...
**SIWX** is developed to work as a plugin system for Reown and to enable correctly it you need to fulfill the expected interface within the `createAppKit` function.
...
All the typings used are exposed by `@reown/appkit-core` package. You may check the source code [here](https://github.com/reown-com/appkit/blob/main/packages/controllers/src/utils/SIWXUtil.ts).
...
You are able to implement the `SIWXConfig` in the way you would like, but some constraints MUST be followed to make sure that Reown can interact with it correctly and it will work as expected:
