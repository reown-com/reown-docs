...
Metadata for your Reown. The `name`, `description`, `icons`, and `url` are used at certain places like the wallet connection, sign message, etc screens. If not provided, they will be fetched from the metadata of your website's document object.
...
It allows you to configure the default account selected for the specified networks in Reown. For example, if you want your EVM networks to use an EOA account by default, you can configure it as shown in the code below.
...
Enable or disable fullscreen rendering of Reown Core on mobile devices. Default is `false`.
...
Enable or disable debug mode in your Reown. This is useful if you want to see UI alerts when debugging. Default is `false`.
...
Enable or disable the swap feature in your Reown. [Swaps](/appkit/react/transactions/swaps) feature is enabled by default.
...
Enable or disable the onramp feature in your Reown. [Onramp](/appkit/react/transactions/onramp) feature is enabled by default.
...
| Value         | Description                                              |
| ------------- | -------------------------------------------------------- |
| `SHOW`        | Shows the "All Wallets" button on Reown.                |
| `HIDE`        | Removes the "All Wallets" button from AppKit.            |
| `ONLY_MOBILE` | Shows the "All Wallets" button on Reown only on mobile. |
