...
To test the complete exchange flow in a local development environment:

1. Clone the example repository:
   ```bash
   git clone https://github.com/reown-com/appkit-web-examples.git
   cd appkit-web-examples/react/react-wagmi-appkit-pay
   ```
...
This allows you to test the complete Pay with Exchange flow using `baseSepoliaETH` in a fully functional application environment.
