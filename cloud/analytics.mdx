...
Indicates the daily average of connections made through Reown.
...
This table and chart shows the count of various events that are triggered as the users interact with Reown.
...
| **AppKit:Session** | A session within the context of Reown analytics represents the connection established between your project and your user’s device (includes browsers). Sessions are created when the user interacts with Reown on your app. If user events are tracked within a 30-minute range, they will be considered within the same session. |
